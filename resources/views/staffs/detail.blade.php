<x-app-layout>
    ...
    <div class="w-full lg:w-3/6 m-6">
        <span class="flex items-center space-x-4">
            <h1 class="text-5xl font-bold leading-tight">Edit Staff</h1>
        ...
        <div class="bg-white shadow-md p-5">
            ...
            <form method="POST" autocomplete="off" action="{{ route('staffs.update',$user->user_id) }}" enctype="multipart/form-data">
                @csrf
                @method('PUT')
                ...
                <div class="mt-4">
                    ...
                    <select name="department" id="idDepartment" class="w-full h-10 pl-3 pr-6 text-base placeholder-gray-600 border rounded-lg apperance-none focus:shadow-outline">
                        @foreach ($departments as $department )
                            <option value="{{ $department->id }}" @if($department->id == $user->department_id) selected @endif>{{ $department->dep_name }}</option>
                        @endforeach
                    </select>
                </div>
                ...
                <div class="mt-4">
                    ...
                    <input value="" type="password" name="password" id="idPassword" class="w-full h-10 pl-3 pr-6 text-base placeholder-gray-600 border rounded-lg appearance-none focus:shadow-outline" placeholder="Password will not be changed if empty" required>
                </div>
